...
The complete required coverage and automation status live in [scenes.json](scenes.json). `startup-ui`, `ui-clipping-opacity`, `text-matrix`, `particles-blends`, `outfit-masks`, `temporary-framebuffers`, `composition-all`, `graph-lines`, and the fixture-backed `map-core` and `map-screenshot` captures are automated now. Entries marked `fixture-server`, `client-script`, `native-fixture`, or `desktop-driver` are the remaining Phase 0 fixture work, not optional coverage.
...
```sh
for scene in ui-clipping-opacity text-matrix particles-blends outfit-masks temporary-framebuffers composition-all graph-lines; do
  DISPLAY="$DISPLAY" build/macos-release/bin/otclient \
