...
- Compare images produced by the same backend and environment. Cross-backend comparisons use the same tolerances but are diagnostic until parity is signed off. Since Phase 4 there is one structured exception: `tools/compare_render_backends.sh` compares OpenGL against Metal *on the same compiled frame*, which is a narrower and answerable question - see below.
