...
The complete required coverage and automation status live in [scenes.json](scenes.json). `startup-ui` is automated now. Entries marked `fixture-server`, `client-script`, `native-fixture`, or `desktop-driver` are the remaining Phase 0 fixture work, not optional coverage.
