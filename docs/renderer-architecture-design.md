...
- **Retained targets** — the MAP and FOREGROUND pool targets. Persist across frames; their texture is sampled by a later composition draw *and may be reused without re-rendering* when the pool hash is unchanged. Resized on viewport/scale change. **Corrected 2026-08-25 (`6b03c256`):** ~~FOREGROUND at `viewport/scale`~~ — the FOREGROUND target is sized at the full device-pixel viewport with the display density as its *content scale*, so its geometry stays in logical units while it rasterises across every physical pixel. Sizing it at `viewport/scale` is what made the whole UI composite at 1x and upscale on a scaled display `[S 9.7]`.
...
struct RenderPass {
    ...
    Color clearColor;
    Rect viewport;                       // the device-pixel viewport (top-left)
    ...
    // pays for itself the first time a GPU capture or a golden diff is read by a person, and,
    // since 2026-08-25 (`b58e805d`), `Size projectionExtent` with a `projectionSize()` accessor.
    // `viewport` alone could not describe a target addressed in one coordinate space and
    // rasterised in a denser one — the FOREGROUND target on a scaled display. An invalid
    // `projectionExtent` means "same as the viewport", which keeps it free for every other pass.
};
```
...
- The projection matrix (`painter.cpp:297-316`) moves into the backends. GL keeps the y-flipping matrix for the backbuffer and uses a *non-flipping* variant for FBO passes, absorbing today's `upsideDown` texture-matrix mechanism; Metal uses one convention everywhere. **Delivered 2026-08-21 (Phase 4):** `MetalBackend` carries the identical projection — Metal's clip space has the same handedness as GL's in x and y, so the same nine numbers are correct — and implements "one convention everywhere" by deriving each texture matrix from the texture's size and never resolving a `textureMatrixId`, because a Metal render target stores row 0 at the top exactly like an uploaded image `[S 7]`. **One exception since 2026-08-25 (`0ec21a80`), and it is in the fragment stage rather than the vertex one:** the coordinate handed to a *module* fragment is converted back into GL's space for render-target sources — see the bullet below.
- `Texture::setUpsideDown` and flipped-blit quads (`addHorizontally/VerticallyFlippedQuad`) leave shared code; the compiler emits pre-flipped UVs where `fbFlip` demands it, and only the GL backend knows render-target textures are stored bottom-up. **Qualified 2026-08-25 (`0ec21a80`): the *storage* convention still divides that way, but the shader-visible coordinate does not.** GL samples a render target through an `upsideDown` texture matrix, so `v_TexCoord.y` runs `1 - t` there where Metal's plain matrix makes it run `t`. Both fetch the right texel; a shader that uses the coordinate as a position rather than only as a sampling coordinate does not. The difference is absorbed in the translation layer, not by unifying storage: `tools/generate_metal_shaders.py` routes every read through `crystalotc_texCoordGL()` and every `u_Tex0` fetch through `crystalotc_sampleTex0()`, gated on a per-draw `u_Tex0FlipY` set from whether the resolved handle names a render target. It cannot be a compile-time constant — the same material binds at both kinds of site.
...
Backend selection is explicit config (`graphics.renderBackend`, `configmanager.h:42`), resolved with the same precedence as the render path — startup flag, then environment, then config — in `requestedRenderBackend()` (`drawpoolmanager.cpp:71-90`). `createBackend()` is the fallback policy: Metal when asked, or when the default `auto` meets a window with no GL context; GL otherwise, and GL again if Metal refuses to initialise. **Extended 2026-08-21 (`219e05b`): it validates the request first.** A named backend this build cannot provide is downgraded to `auto` rather than attempted, against the compile-time list `DrawPoolManager::availableRenderBackends()` reports. Without that, `--render-backend=gl` on a Cocoa window produced a client that drew *nothing* - `GLBackend::initialize()` fails with no GL context, `createBackend` returned null, the frame path was refused, and the legacy path it fell back to is the OpenGL renderer. `requestedRenderBackend()` also honours `opengl` and `auto` from config now, while `gl` deliberately keeps meaning `auto`: it is the built-in default and the Vulkan feeder's not-vulkan value, so reading it as a deliberate OpenGL request would make every machine that has never opened the options screen demand a backend it may not have.
...
| `FrameBuffer` | **dies** after migration (shim during). **Not yet, as of 2026-08-21 (Phase 4):** still owns every *OpenGL* render target. Under Metal the target texture belongs to `MetalResources::ensureTarget` and the `FrameBuffer` survives only as the size-, filtering- and (since 2026-08-25, `b58e805d`) content-scale descriptor the registry and the compiler resolve to, which is how the smoothness policy stays in one place and how a target's coordinate space stays attached to the target rather than to the frame. `ResourceRegistry::bindTarget` still maps handles to `FrameBuffer*` |
...
5. **Settled from source 2026-08-20 (Phase 1), recorded here because §3 and §9 depend on it:** the platform layer reports `m_size` in **backing pixels** and `m_displayDensity` as the **effective logical-to-device scale** — since 2026-08-25 the product of the backing scale factor (`getDevicePixelRatio()`) and the user's HUD scale (`getHUDScale()`), and that factor alone when this was written, following the `AndroidWindow` precedent. This is forced by `GraphicalApplication::resize`, which feeds `m_size` to `g_graphics` (and thence `glViewport`/`Painter::setResolution`) while laying the UI out at `m_size / m_displayDensity`. `RenderFrame::drawableSize` and `IRenderBackend::resize` are therefore already in the right unit. ~~**Caveat the design must not inherit silently:** `g_app.setHUDScale` writes the *same* variable, so device pixel ratio and user HUD scale are conflated; separating them is a framework change, not a backend one.~~ **Resolved 2026-08-25 (`07b9597d`), and Phase 1 classified it correctly — it was a framework change, not a backend one.** `PlatformWindow` holds `m_devicePixelRatio` and `m_hudScale` as separate inputs and caches their product in `m_displayDensity`, so `getDisplayDensity()` keeps its meaning and no consumer changed. Both setters go through `g_app` rather than `g_window`, because either change has to relayout the interface. **Consequence measured 2026-08-21 (Phase 5), having been recorded three times as a classification and never as work:** because the FOREGROUND target is sized by that same conflated value and then blitted to the full physical viewport, a window with a backing scale of 2 renders the entire UI at half resolution and upscales it. **Fixed 2026-08-25 (`b58e805d`, `6b03c256`).** `FrameBuffer` gained a content scale and `RenderPass` a `projectionExtent`, so a target can be addressed in one coordinate space and rasterised in a denser one; the FOREGROUND target is sized in device pixels while the UI keeps laying out in logical units, and the composition blit is 1:1. Measured on `windowing`'s HUD-scale-2 capture: mean absolute horizontal gradient 3.702 → 5.193 at alignment (0,0). Measured locally; CI is suspended. See the follow-up in `docs/phase-5-renderer-handoff.md`; it belonged to no phase in this plan and was done outside one, 2026-08-25.
