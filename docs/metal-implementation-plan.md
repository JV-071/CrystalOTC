...
**Status:** Implementation plan (draft for review)
**Date:** 2026-08-19
**Companions:**
...
- The OpenGL behavioral reference is captured **locally via the XQuartz/GLX path** (the companion doc's path 1, promoted from "developer fallback" to required reference vehicle) and **canonically via CI software rendering** (Linux runner + Xvfb + Mesa llvmpipe: deterministic, reproducible images with no GPU).
...
- The RecordingBackend golden-frame suite `[D §9.1]` is platform-independent (PoolCompiler output is identical on every OS) and becomes the primary cross-platform correctness instrument.
...
```
P0 Baselines ──► P2 Renderer boundary ──► P3 GL on RenderFrame ──► P4 Metal foundation ──► P5 Targets & composition ──► P6 Materials ──► P7 Hardening
                     ▲
P1 Cocoa platform ───┘  (P1 runs in parallel with P0/P2; P4 needs P1+P3 done)
```

- **P1 ∥ P0/P2:** the Cocoa window needs no renderer work and the boundary needs no macOS.
...
Tasks:
1. **XQuartz GL bring-up** (companion doc path 1, scoped "small to medium"): resolve macOS vcpkg dependencies, fix compiler/linker incompatibilities, validate GLX context creation on Apple Silicon, run the client under XQuartz. This is the local day-to-day reference for every later phase.
2. **CI software-GL reference**: Linux CI job with Xvfb + Mesa llvmpipe rendering the validation-matrix scenes headlessly and archiving the images. llvmpipe output is deterministic across runs and machines — these become the canonical reference set (Windows-GL capture is replaced by this; spot-check against a community Windows screenshot when one is available).
3. Script the validation-matrix scenes (companion doc, Validation matrix): login screen, map with floors/creatures/missiles, day/night + colored overlapping lights, translucent nested-clipped UI, bitmap/TTF/outlined text, outfit masks + mounts + addons, particles, each live composition mode `[S 2.2]`, resize/fullscreen.
4. Add scenes that exercise every surveyed edge: the seven temp-FBO sites `[S 3.2]`, the map-hole punch `[S 4]`, `Outfit - Outline` (`useFramebuffer`), Fog/Snow (multi-texture), UIGraph lines `[S 6.3]`, atlas growth, map screenshot.
5. Resolve open question `[D §12.4]`: inspect the `glReadPixels(x/3, y/1.5)` map-screenshot offsets on the XQuartz build (bug vs. intended framing); record the verdict in the survey doc.
6. Record frame-time/memory baselines (existing `AUTO_STAT` counters suffice initially; XQuartz numbers are for relative comparison only — its GL stack is not representative of Windows performance).

**Exit gate:** the client runs on macOS via XQuartz; a checked-in scene list + CI-generated reference-image set + a one-page "known deviations" note (including any XQuartz-vs-llvmpipe rendering differences observed).
...
3. Platform selection: route macOS to `CocoaWindow` in `src/framework/platform/platformwindow.cpp`; drop the unconditional X11 requirement for Apple in CMake.
...
4. Image-comparison harness: new-path output vs Phase 0 references. Two comparisons, both same-environment so no cross-platform noise pollutes the gate: locally, legacy-path vs new-path on XQuartz; in CI, legacy-path vs new-path on llvmpipe (small tolerance; hard-fail on missing passes/clip/coordinate errors, per the companion doc's validation-matrix note).
...
**Goal:** the complete frame pass graph `[S 10]` runs on Metal; output matches the Phase 0 reference scenes within tolerances.
...
**Exit gate:** companion doc Phase 5 criterion — Metal matches the OpenGL reference scenes apart from documented tolerances; performance within baseline envelope.
...
2. **Inventory port**: the ~21 module fragments `[S 5.3]` — validate each against its GL rendering in the matrix scenes; multi-texture binding (`u_Tex1..3` → texture slots) for Fog/Snow.
...
4. **GL side of the ABI**: map `MaterialParams` fields onto the legacy uniform locations so existing `.frag` sources compile unchanged; retire the index-10 collision `[S 9.4]` and record the visual verdict of whatever it did before.
...
6. CI: macOS build + smoke + golden frames + image matrix as a required job.
...
| Pixel-parity gate too strict for legitimate GL/Metal sampling differences | P3/P5 | tolerance policy defined in P0 harness, per companion doc validation note (small color/sampling tolerance; structural differences hard-fail) |
...
| Atlas padding trick behaves differently on Metal clamp | P5 | dedicated matrix scene; fall back to CPU-padded upload if sampling differs `[S 9.6]` |
| XQuartz/GLX bring-up on Apple Silicon stalls (P0.1) | P0 | llvmpipe CI reference works without it; worst case the reference is CI-only and local iteration leans on golden frames + the CI image diff |
| XQuartz or llvmpipe GL output deviates from real Windows GL | P0/P3 | GL2-level feature set makes driver divergence unlikely; "known deviations" note; spot-check against community Windows screenshots |

## Definition of done

The macOS milestone is complete when: the `.app` runs the full validation matrix with Metal output matching Phase 0 references within the tolerance policy; golden-frame and image-comparison suites are required CI; the llvmpipe CI job proves GL-on-`RenderFrame` parity (standing in for Windows/Linux runtime validation); Windows and Linux CI builds compile green with the Vulkan feeder behavior-unchanged by construction; and the shader toolchain rejects untranslatable GLSL at build time.
