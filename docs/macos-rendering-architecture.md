...
- Keep Windows/Linux behavior stable. **Unchanged by construction:** the frame path is off by default and the Vulkan feeder still consumes the same published `DrawObject` lists. **Qualified 2026-08-21 (Phase 4):** both halves weakened. A window with no GL context now resolves to the frame path regardless of the default — and the Windows Vulkan path is such a window, so it walks that code, finds no backend and logs two new startup warnings before settling back on the legacy path, with the draw loop unaffected because `VkDrawFeeder` still runs under the `#ifdef WIN32` arm. And Phase 4 changed shared no-GL code that path executes: `ShaderProgram` identity and hashing, and `AnimatedTexture::update`, which now advances animations there that previously held on frame 0. **And Phase 6 changed it again:** `ShaderManager` registers module programs without a GL context and skips only compilation, so that path now carries non-null `PoolState::shaderProgram`, a different `DrawPool` state hash, and the `Outfit - Outline` `useFramebuffer` route. All four are recorded in the implementation plan's risk register.
...
~~Success criterion: supported effects and outfits behave consistently across OpenGL, Vulkan, and Metal.~~ **Met for OpenGL and Metal on the offline matrix, locally, 2026-08-21; QUALIFIED for map shaders; the Vulkan leg is not met and was not attempted.** All eleven offline scenes compare across the two backends now, eight at 0 differing pixels. The qualification came from `shader-matrix-map`, comparable for the first time in the migration: six of its fourteen map shaders differ substantially, all of them shaders that read `v_TexCoord` as a spatial coordinate, because GL samples a render target through an `upsideDown` texture matrix while Metal stores targets top-down — the sampled texel agrees, the coordinate value is mirrored. Vulkan is untouched by design — `VkDrawFeeder` still ignores painter shader programs, as this document records above — and Phase 6 in fact *changed* that path's registration behaviour rather than its shading. Nothing is pushed, so no CI job has verified any of this.
