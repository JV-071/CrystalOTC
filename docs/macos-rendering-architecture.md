...
- ~~Compare output and performance against Phase 0.~~ **Output compared 2026-08-21 (`1902851`):** `tools/compare_render_paths.sh` captures every offline scene down both paths and compares them; ten of eleven match at 0 differing pixels, the eleventh being the deliberately divergent line scene. **Performance is still outstanding** — it was deferred here from Phase 0 because that is where the two paths can be measured in one environment, and it has not been measured.
