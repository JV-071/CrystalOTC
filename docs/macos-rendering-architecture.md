...
**Resolved 2026-08-21 (Phase 6): policies 3 and 4, not 2.** The closed repo-shipped set takes policy 3, but *offline* rather than "during the build" — `tools/generate_metal_shaders.py` runs glslang and SPIRV-Cross over the set declared in `tools/metal_materials.json`, and the MSL is **committed** as a generated header that the build and CI verify rather than produce; each material's library is compiled at runtime on first use. Arbitrary runtime GLSL takes policy 4: `createFragmentShaderFromCode` registers with an empty source key and Metal draws it with the built-in Textured material, logging once. Policy 2 survives as the shape of the material *vocabulary* — `BuiltinMaterial` plus the frozen `MaterialParams` block — but it is not how module shaders are implemented. Author-facing consequences are in `docs/shader-authoring.md`.
...
**Status 2026-08-20: met, ~~with one qualification about where it is enforced~~ — and as of 2026-08-21 without it.** `RenderGoldenFrame.RepresentativeFrameMatchesTheBaseline` compiles a representative pool — direct draws, a composition-mode bracket, a nested transient target with a flip, a map-hole punch and a live scissor — into three passes and six packets, and diffs the recording against a checked-in golden on a runner with no GPU. ~~The qualification: `ctest` runs in exactly one workflow, `.github/workflows/build-macos.yml`. `build-windows.yml` and `render-baseline-linux.yml` compile the new suite but never execute it, so the golden frame is gated on macOS alone.~~ **Corrected 2026-08-21 (Phase 2, `2784e73`):** all three build jobs run `ctest` — `build-macos.yml:128`, `build-windows.yml:132`, `render-baseline-linux.yml:205` — so both golden frames (`RepresentativeFrameMatchesTheBaseline` and `StructureViewIsStableToo`) are gated on AppleClang/libc++/arm64, MSVC/x86-64 and GCC/libstdc++/x86-64. Observed in runs `32440662987`, `32440663105` and `32440662865`.
...
- Keep Windows/Linux behavior stable. **Unchanged by construction:** the frame path is off by default and the Vulkan feeder still consumes the same published `DrawObject` lists. **Qualified 2026-08-21 (Phase 4):** both halves weakened. A window with no GL context now resolves to the frame path regardless of the default — and the Windows Vulkan path is such a window, so it walks that code, finds no backend and logs two new startup warnings before settling back on the legacy path, with the draw loop unaffected because `VkDrawFeeder` still runs under the `#ifdef WIN32` arm. And Phase 4 changed shared no-GL code that path executes: `ShaderProgram` identity and hashing, and `AnimatedTexture::update`, which now advances animations there that previously held on frame 0. **And Phase 6 changed it again:** `ShaderManager` registers module programs without a GL context and skips only compilation, so that path now carries non-null `PoolState::shaderProgram`, a different `DrawPool` state hash, and the `Outfit - Outline` `useFramebuffer` route. All of them are recorded in the implementation plan's risk register, which now carries six entries.
...
- Implement standalone textures and the sprite array atlas. **Standalone textures done 2026-08-21; the CPU atlas done 2026-08-21 (Phase 5, `83e81ac`).** ~~The CPU atlases are switched off under Metal, because `TextureAtlas` keys its regions on a texture's OpenGL name — zero for every texture a non-GL backend creates — and `TextureAtlas::flush` is unguarded OpenGL throughout. Atlas layers become explicit passes in Phase 5.~~ Both obstacles were removed: regions are keyed on the process-wide unique id, and maintenance compiles to ordinary `Keep`-loaded passes with blend-off packets. Note this is the *CPU* atlas shared with the GL path, not the `MTLTextureType2DArray` sprite array this section sketches — that remains unbuilt and unneeded.
- ~~Implement scissor, transforms, opacity, and normal/multiply blending.~~ **Done**, and all six composition modes rather than two.

~~Success criterion: login UI and normal gameplay render with parity to the current Vulkan path.~~ **Superseded by Decision 1 (full OpenGL parity, not Vulkan parity) and met against that instead, 2026-08-21.** Measured by capturing each offline scene from an XQuartz binary and a Cocoa binary with both forced onto the compiled frame path, so the two consume an identical `RenderFrame`: six of the nine comparable scenes match at 0 differing pixels of 656,880, two differ only by a single Outline probe (579 and 521 px, against a stated 0.002 envelope), and two are excluded as not comparable until the Phase 6 shader toolchain exists. **Superseded 2026-08-21 (Phase 6):** the toolchain exists, both exclusions and both 0.002 tolerances are gone from the manifest, and all eleven offline scenes compare — eight at 0 differing pixels, including the two Outline probes recorded here at 579 and 521 px.
...
~~Success criterion: Metal output matches the established OpenGL reference scenes apart from documented tolerances.~~ **Met 2026-08-21**, against the *live* OpenGL backend rather than a frozen macOS reference set, which was found not to be needed: both sides consume an identical `RenderFrame`. Offline, six of nine comparable scenes at 0 differing pixels; online, every scene agreeing within its own noise, with `map-core` at 0 px of 656,880. **Superseded 2026-08-21 (Phase 6):** the population changed with it — all eleven offline scenes compare now, eight at 0 differing pixels, with `outfit-masks` and `temporary-framebuffers` moving from 579 and 521 px to 0.

## Phase 6: Shader and material migration

- ~~Inventory every built-in and module-created shader.~~ **Done 2026-08-21 (`ec953ac`):** the closed set is declared in `tools/metal_materials.json` — 23 of the 24 shipped `.frag` translated, `test.frag` the only exclusion and carrying a stated reason.
- ~~Define the supported semantic material set.~~ **Done:** `BuiltinMaterial` plus one material per translated `.frag`, resolved by source-file basename through the new `MaterialRegistry`, because several registered names share one file.
- ~~Add MSL implementations.~~ **Done 2026-08-21 (`ec953ac`):** generated offline by glslang and SPIRV-Cross into the committed header `src/framework/graphics/render/metal/metalmodulematerials.h`, compiled at runtime one `MTLLibrary` per material, lazily on first use. There is no `.metallib`.
- ~~Decide the policy for arbitrary runtime GLSL.~~ **Decided and implemented 2026-08-21 (`33961ca`):** unsupported on Metal by design — `createFragmentShaderFromCode` registers with an empty source key, so the backend draws it with the built-in Textured material and says so once. Written up for module authors in `docs/shader-authoring.md`.
- ~~Add shader/pipeline compilation diagnostics~~ **done 2026-08-21 (`ef9de16`)** — per-material one-time logging plus compile-failure, missing-entry-point and pipeline-creation warnings — **and caching: deliberately in memory only.** Compilation is lazy and per material, so an on-disk pipeline cache has nothing to save; revisit in Phase 7 if a cold bind ever shows up in a measurement.

~~Success criterion: supported effects and outfits behave consistently across OpenGL, Vulkan, and Metal.~~ **Met for OpenGL and Metal on the offline matrix, locally, 2026-08-21; QUALIFIED for map shaders at that date and MET unqualified since 2026-08-25; the Vulkan leg is not met and was not attempted.** All eleven offline scenes compare across the two backends now, eight at 0 differing pixels. The qualification came from `shader-matrix-map`, comparable for the first time in the migration: six of its fourteen map shaders differ substantially, all of them shaders that read `v_TexCoord` as a spatial coordinate, because GL samples a render target through an `upsideDown` texture matrix while Metal stores targets top-down — the sampled texel agrees, the coordinate value is mirrored. **Qualification removed 2026-08-25 (`0ec21a80`):** the divergence is absorbed in the shader translation layer — the fragment's arithmetic runs in GL coordinate space and converts once at the `u_Tex0` fetch, gated per draw on whether that texture is a render target — so all fourteen `shader-matrix-map` captures now sit at or below the scene's unshaded control frame, and Metal keeps its top-down targets. The Vulkan leg is still not met and still not attempted. Vulkan is untouched by design — `VkDrawFeeder` still ignores painter shader programs, as this document records above — and Phase 6 in fact *changed* that path's registration behaviour rather than its shading. ~~Nothing is pushed, so no CI job has verified any of this.~~ **Corrected: Phase 6 was pushed and all four CI jobs verified it, `994371b` included.** CI has since been suspended (2026-08-25, Actions minutes exhausted), so the map-shader fix layered on top of this is local-only evidence — see the implementation plan's development-environment constraints.
...
XQuartz is not the target native macOS architecture, but it is no longer optional: it is the required local OpenGL reference vehicle for the Metal migration, and a hard requirement of the default macOS CMake path (`src/CMakeLists.txt:220-232`).
