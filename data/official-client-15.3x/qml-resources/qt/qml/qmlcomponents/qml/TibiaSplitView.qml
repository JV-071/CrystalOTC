...
/*!
    ...
    TibiaSplitView is a control that lays out items horizontally or
    vertically with a draggable splitter between each item.

    There will always be one (and only one) item in the TibiaSplitView that has \l{Layout::fillWidth}{Layout.fillWidth}
    ...
    By default, the last visible child of the TibiaSplitView will have this set, but
    ...
    TibiaSplitView supports setting attached Layout properties on child items, which
    ...
    \note Please import QtQuick.Layout in your QML file in order to use the Layout
    attached properties inside TibiaSplitView.

    Example:

    To create a TibiaSplitView with three items, and let the center item get superfluous space, one
...
Item {
    ...
    /*!
        \qmlproperty enumeration TibiaSplitView::orientation

        This property holds the orientation of the TibiaSplitView.
...
*/
    ...
    */
    ...
    /*! \qmlmethod void TibiaSplitView::addItem(Item item)
    ...
    /*! \qmlmethod void TibiaSplitView::removeItem(Item item)
    ...
    QtObject {
        ...
        function updateLayout()
        {
            ...
            // Special case: If TibiaSplitView size is zero, we leave fillItem with the size
            // it already got, and assume that TibiaSplitView ends up with implicit size as size:
            if (root[d.size] != 0) {
            ...
            MouseArea {
                ...
                onEntered: {
                  if(tibiaMouseCursorController != null) {
                    tibiaMouseCursorController.setResizeVertical(root.orientation === Qt.Vertical);
                    tibiaMouseCursorController.setResizeHorizontal(root.orientation === Qt.Horizontal)
                  }
                }
                onExited: {
                  if(tibiaMouseCursorController != null && !pressed) {
                    tibiaMouseCursorController.setResizeVertical(false);
                    tibiaMouseCursorController.setResizeHorizontal(false);
                  }
                }
                onReleased: {
                  if(tibiaMouseCursorController != null) {
